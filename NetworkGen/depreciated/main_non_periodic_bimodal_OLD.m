...
%% --------------------------- USER INPUTS --------------------------------
% Kuhn length 
b = 1.6;                    % constant Kuhn length 1.6 (nm)

L0x = 1300;                  % system size x-dir
L0y = 800;                  % system size y-dir

Nettype = 5;                % Network type: Navg = [500 1000 2500 5000 10000]

% Output files
lammps_data_file   = 'PronyNetwork_10000_nano_1300x800.dat';
lammps_visual_file = 'PronyVisual_10000_nano_1300x800.dat';
bond_table_file    = 'bond.table';

%% ------------------------- NETWORK PRESETS ------------------------------

N1vec = [50 75   80  100  200];
N2vec = [50 200 500 1000 2000];
Rcut = [6 6 8 10 10];
Pvec = [0.1 0.2 0.3 0.3 0.4];
%% --------------------------- SYSTEM SETUP -------------------------------
% Domain (2D)
xlo = -L0x*b;  xhi = L0x*b; %
ylo = -L0y*b;  yhi = L0y*b; %
zlo =  -20*b;  zhi = 20*b;  %     % thin z just to satisfy LAMMPS format

% Bond Khun segments (assume ~10 monomers/Kuhn segment)
% Navg = [500 1000 2500 5000 10000] % monomer average
N1 =  N1vec(Nettype);  %[50 75   80  100  200]
N2 =  N2vec(Nettype); %[50 200 500 1000 2000]
...
% probability for type 2 bonds
% P = [0.1 0.2 0.3 0.3 0.4]
P2 = Pvec(Nettype);                    % Probability constraint to reduce number of long bonds

% Bond cutoff ranges
r1_lower_cutoff = r1_avg - FWHM1/2;     %type 1         
r1_upper_cutoff = r1_avg + FWHM1/2;

r2_lower_cutoff = r2_avg - FWHM2/2;     %type 2
...
% Node scattering (min distance) + guards
                            %[0.3 0.3 0.4 0.5 0.5] or [6 6 8 10 10]*b
min_node_sep               = Rcut(Nettype)*b;       % HARD minimum spacing between any 2 nodes 0.30
...
% %% ----------------- Iteratively remove excess bonds ----------------------
% % Attempts to achieve a target monomer density by removing excess bonds
% 
% type1 = Bonds(:,5) == 1;
% type2 = Bonds(:,5) == 2;
% Ntarget_kuhn = min_node_sep2*N_atom/(b/2)^2;
% Ntotal_kuhn = N1*sum(type1) + N2*sum(type2);
% 
% min_peratom_bond = 3;                       %dont delete if atom will have less than three bonds
% max_bondremoval  = round(0.2*Max_bond);     %dont delete more than 10% of the network
% 
% attempts_without_progress = 0;
% bonds_removed             = 0;
% 
% while Ntotal_kuhn > Ntarget_kuhn && bonds_removed < max_bondremoval
% 
%     if attempts_without_progress >= max_attempts_without_progress
%         break;
%     end
%     % select random bond
%     id_random = randi(Bond_count,1);
%     %idelete = 0;
% 
%     i1 = Bonds(id_random,2);
%     i2 = Bonds(id_random,3);
%     type = Bonds(id_random,5);
% 
%     if type == 1 && rand() < sum(type2)/sum(type1)
%         attempts_without_progress = attempts_without_progress + 1;
%         continue
%     end
% 
%     if (Atoms(i1,5) <= min_peratom_bond) || (Atoms(i2,5) <= min_peratom_bond)
%         attempts_without_progress = attempts_without_progress + 1;
%         continue
%     end
% 
...
% 
%     % Remove their references from neighbors and decrement counts
%     % For every atom, if neighbor == jj, remove it
%     for jj = [i1 i2]
%             for ii = [i1 i2]
%                 if Atoms(ii,1) == 0, continue; end
%                 nb = Atoms(ii,5);
...
Rcut = sqrt(Ntotal_kuhn/Atom_count*(b/2)^2);

Domain_Area = (xhi-xlo)*(yhi-ylo);
Kuhn_Density = Ntotal_kuhn/Domain_Area;

fprintf("Cutoff radius %4.4f\n",Rcut);
fprintf("Areal Density of Kuhn segments %4.4f\n",Kuhn_Density);
fprintf("%4.2f percent type 1 bonds | Average Kuhn segments/chain: %4.2f \n",100*sum(type1)/length(type1),Nkuhn_avg)
...
fprintf(fid, '%1.10f %1.10f xlo xhi\n', 2*xlo, 2*xhi);
fprintf(fid, '%1.10f %1.10f ylo yhi\n', 2*ylo, 2*yhi);
fprintf(fid, '%1.10f %1.10f zlo zhi\n', zlo, zhi);
fprintf(fid, '\n');

fprintf(fid, 'Atoms #bpm/sphere\n\n');
for i = 1:Atom_count
    % Format: atomID  molID  atomType  x y z
    fprintf(fid, '%d 1 1 1 0.1 %1.10f %1.10f %1.10f\n', All_Atoms(i,1), All_Atoms(i,2), All_Atoms(i,3), All_Atoms(i,4));
end
...
for i = 1:Atom_count
    ...
    fprintf(fid, '%d 1 %1.10f %1.10f %1.10f 1 1 1 \n', All_Atoms(i,1), All_Atoms(i,2), All_Atoms(i,3), All_Atoms(i,4));
end
