...
% ----------------- Thermo source selection -----------------
% 'auto'          : try harmonic first (if exists), else stress*.txt
% 'stress'        : force stress*.txt
% 'harmonic'      : force *harmonic.out
thermo_source = 'harmonic';

% Pattern for harmonic thermo (inside each directory)
harmonicPattern = '*harmonic.out';

% For harmonic files, sigma_yy = -Pyy (col 4), lambda_y is col 2 directly
harmonic_lambda_col = 2;
harmonic_pyy_col    = 4;
...
for p = 1:nPairs
    ...
    %% ---- 1) Read thermo (UNNOTCHED) for stress-stretch ----
    curDir    = unDir;
    wantLabel = 'Unnotched';

    % --- decide which thermo file to use ---
    d_h = dir(fullfile(curDir, harmonicPattern));
    d_s = dir(fullfile(curDir, thermoPattern));

    useH = false; useS = false;
    switch lower(thermo_source)
        case 'harmonic'
            useH = ~isempty(d_h);
            if ~useH
                error('Pair %d (%s): No harmonic thermo matching "%s" in %s', ...
                    p, wantLabel, harmonicPattern, curDir);
            end
        case 'stress'
            useS = ~isempty(d_s);
            if ~useS
                error('Pair %d (%s): No stress thermo matching "%s" in %s', ...
                    p, wantLabel, thermoPattern, curDir);
            end
        case 'auto'
            if ~isempty(d_h)
                useH = true;
            elseif ~isempty(d_s)
                ...
                error('Pair %d (%s): No thermo file found. Looked for "%s" and "%s" in %s', ...
                    p, wantLabel, harmonicPattern, thermoPattern, curDir);
            end
        otherwise
            error('Unknown thermo_source: %s', thermo_source);
    end

    % --- read harmonic.out ---
    if useH
        thermo_used = fullfile(curDir, d_h(1).name);
        fprintf('  %s thermo (harmonic): %s\n', wantLabel, thermo_used);

        fid = fopen(thermo_used, 'r');
        if fid < 0, error('Cannot open thermo file: %s', thermo_used); end

        rows = [];
        tline = fgetl(fid);
        while ischar(tline)
            s = strtrim(tline);
            if ~isempty(s) && ~isempty(regexp(s, '^[\-\+]?\d', 'once'))
                toks = regexp(s, '\s+', 'split');
                % harmonic format has 14 columns
                vals = nan(1,14);
                m = min(numel(toks),14);
                for j = 1:m
                    v = str2double(toks{j});
                    if ~isnan(v), vals(j) = v; end
                end
                % require stime + lambda_y
                if ~isnan(vals(1)) && ~isnan(vals(harmonic_lambda_col))
        ...
        if isempty(rows)
            error('No numeric thermo rows found in %s', thermo_used);
        end

        Step_cur   = rows(:,1);  % stime
        lambda_cur = rows(:,harmonic_lambda_col); % already lambda_y
        Pyy_cur    = rows(:,harmonic_pyy_col);

        good = isfinite(Step_cur) & isfinite(lambda_cur) & isfinite(Pyy_cur);
        Step_cur   = Step_cur(good);
        lambda_cur = lambda_cur(good);
        Pyy_cur    = Pyy_cur(good);

        [~, idxLast] = unique(Step_cur, 'last');
        keep = false(size(Step_cur)); keep(idxLast) = true;
        Step_cur   = Step_cur(keep);
        lambda_cur = lambda_cur(keep);
        Pyy_cur    = Pyy_cur(keep);

        [Step_cur, ord] = sort(Step_cur);
        lambda_cur = lambda_cur(ord);
        Pyy_cur    = Pyy_cur(ord);

        sigma_cur = -Pyy_cur; % tension positive (same convention)

    % --- read stress*.txt (your existing format) ---
    else
        thermo_used = fullfile(curDir, d_s(1).name);
        fprintf('  %s thermo (stress): %s\n', wantLabel, thermo_used);
        ...
        good = isfinite(Step_cur) & isfinite(Ly_cur) & (isfinite(cvir2_cur) | isfinite(cvirs2_cur));
        Step_cur   = Step_cur(good);
        cvir2_cur  = cvir2_cur(good);
        cvirs2_cur = cvirs2_cur(good);
        Ly_cur     = Ly_cur(good);
        ...
        Step_cur   = Step_cur(keep);
        cvir2_cur  = cvir2_cur(keep);
        cvirs2_cur = cvirs2_cur(keep);
        Ly_cur     = Ly_cur(keep);

        [Step_cur, ord] = sort(Step_cur);
        cvir2_cur  = cvir2_cur(ord);
        cvirs2_cur = cvirs2_cur(ord);
        Ly_cur     = Ly_cur(ord);

        if isempty(Ly_cur) || Ly_cur(1) == 0
            error('Ly(1) empty/zero for %s thermo in pair %d.', wantLabel, p);
        end
        lambda_cur = Ly_cur ./ Ly_cur(1);

        switch lower(stress_choice)
            case 'virial'
                sigma_raw = cvir2_cur;
            case 'reduced'
                sigma_raw = cvirs2_cur;
            otherwise
    ...
    % optional plotting stride
    if plot_stride > 1
        idx = 1:plot_stride:numel(lambda_cur);
        lambda_cur = lambda_cur(idx);
        sigma_cur  = sigma_cur(idx);
    end

    lambda_un = lambda_cur;
    sigma_un  = sigma_cur;
    ...
    %% ---- 2) Read thermo (NOTCHED) for stress-stretch ----
    curDir    = unDir;
    wantLabel = 'Unnotched';
